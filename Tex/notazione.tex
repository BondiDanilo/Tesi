...
$\vec u _\varphi = (-x,y,0)$ Versore angolo azimutale \\
...
\begin{equation}\label{eq:versors}
   \begin{cases}
       ...
       \vec u_\varphi = -\sin\varphi \vec u _x + \cos\theta \vec u _y
   \end{cases}
\end{equation}
...
$S^n = \{ x \in \mathbb{R}^{n+1} \tc |x| = 1 \}$: Sfera n-dimensionale di raggio unitario.

\subsubsection{Gruppi di matrici}
\begin{itemize}
   \item $GL(n,\mathbb{F}) = \{M \in Mat(n,\mathbb{F}) \tc detM \neq 0)\}$\\
      \ttab Gruppo Lineare Generale: Matrici $n \times n$ sul campo $\mathbb{F}$,
      invertibili.

   \item $SL(n,\mathbb{F}) = \{ M \in GL(n,\mathbb{F}) \tc det(M) = 1)\}$\\
      \ttab Gruppo Lineare Speciale: Matrici a determinante 1.

   \item $O(n) = \{ M \in GL(n,\mathbb{R}) \tc  M^T M = M M^T = 1 \}$\\
      \ttab Gruppo Ortogonale: Matrici ortogonali.

   \item $SO(n) = \{ M \in O(n)\tc det(M) = 1  \}$\\
      \ttab Gruppo Ortogonale Speciale: Matrici ortogonali a determinante 1.

   \item $U(n) = \{ M \in GL(n,\mathbb{C})\tc M^dagger M = M M^\dagger = 1\}$\\
      \ttab Gruppo Unitario: Matrici unitarie.

   \item $SU(n) = \{ M \in U(n)\tc det(M) = 1  \}$\\
